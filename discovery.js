...
async function updateDiscoveryList() {
  try {
    ...
    const valid = j.data.filter(t => t.symbol.endsWith("_UMCBL") && !t.symbol.startsWith("USDC") && (+t.usdtVolume > 5000000));
...
function vwap(c){
  let pv=0,v=0; for(const x of c){ const p=(x.h+x.l+x.c)/3; pv+=p*x.v; v+=x.v; } return v?pv/v:null;
}
...
function analyzeCandidate(rec) {
  ...
  if (rec.priceVsVwap > 0.5 && rec.rsi1h > 55 && rec.rsi15 > 50) {
    ...
    if (s >= 70) { direction = "LONG"; score = s; reason = `Vol x${rec.volRatio.toFixed(1)} | RSI Structure Bull`; }
  }
  // SHORT
    ...
    if (rec.change24 < -5) s += 10;
    if (s >= 70) { direction = "SHORT"; score = s; reason = `Vol x${rec.volRatio.toFixed(1)} | RSI Structure Bear`; }
  }
...
async function scanDiscovery(){
  ...
  for(const c of best){
    ...
    const msg = `⚡ *JTF DISCOVERY* ⚡\n\n${emoji} *${c.symbol}* — ${c.direction}\n📊 Score: ${c.score}/100\n💡 ${c.reason}\n\n🔹 Entry: ${c.price}\n🛑 SL: ${c.sl} (-${c.riskPct}%)\n🎯 TP: ${c.tp}\n\n🌪️ Vola: ${c.vola}%\n📢 Volume: x${c.volRatio}\n_Levier faible (2x max)_`;
    await sendTelegram(msg); console.log(`✅ DISCOVERY Signal: ${c.symbol}`);
  }
}

async function main(){
  console.log("🔥 JTF DISCOVERY v0.1 Engine démarré.");
  await sendTelegram("🔥 *JTF DISCOVERY Bot (Mid-Caps) connecté au serveur central*");
  while(true){ try { await scanDiscovery(); } catch(e) { console.error("Discovery Loop Error:", e.message); } await sleep(SCAN_INTERVAL_MS); }
}
