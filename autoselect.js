...
// Version stable — API v2 normalisée, protection null, ESM OK
...
const VALIDATION_DELAY_MS = 0; // gardé pour futur
...
// Normalise les réponses Bitget qui peuvent être soit objet, soit [objet]
...
// Ticker v2 — data peut être un array de 1 élément
...
// Mark-price v2 — on lit markPrice ou indexPrice, en gérant aussi le cas array
...
// Funding v2 — data peut aussi être un tableau
...
// Open Interest v2 — on normalise comme les autres
...
async function processSymbol(symbol){
  ...
  const last =
    tk.lastPr   != null ? +tk.lastPr   :
    tk.markPrice!= null ? +tk.markPrice:
    tk.last     != null ? +tk.last     :
    null;

  const high24 = tk.high24h != null ? +tk.high24h : null;
  const low24  = tk.low24h  != null ? +tk.low24h  : null;

  const openInterest = oi && oi.amount != null ? +oi.amount : null;
...
function buildTradePlan(rec, fusion, jds, rr){
  ...
  // Protection : si pas de prix ⇒ plan vide
  if (!price || price <= 0){
    return {
      entry: null,
      sl:    null,
...
async function scanOnce(){
  ...
  // Filtrer les plans invalides (pas de prix)
